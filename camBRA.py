...
def exibir_logo(logo_path="logo.jpg"):
    ...
    with col3:
        st.markdown(f"<h3 style='text-align: center; font-weight:bold'>Empresa<br>Brasília<br></h3>", unsafe_allow_html=True)
...
def verificar_nota_existente(nota_fiscal):
    ...
    cursor.execute(
        """
        SELECT COUNT(*) FROM NotaFiscaisCanhotoBRA
        WHERE NumeroNota = ?
        """,
        (nota_fiscal,)
    )
...
def salvar_imagem_no_banco(imagem, nota_fiscal):
    ...
    try:
        cursor.execute(
            """
            INSERT INTO NotaFiscaisCanhotoBRA (NumeroNota, Imagem, DataBipe)
            VALUES (?, ?, ?)
            """,
...
def contar_canhotos():
    conn = conectar_banco()
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM NotaFiscaisCanhotoBRA")
...
if pagina == "📸 Captura de Imagem":
    ...
    # Validação para permitir apenas números
    if numero_nota and not numero_nota.isdigit():
        st.error("⚠️ Apenas números são permitidos para o número da nota fiscal. Por favor, insira um valor válido.")
